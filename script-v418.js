...
const CP_DEV_CACHE_BUST = '2026-06-27T0715-v418-admin-live-activity-status-sync';
const BUILD = {
  version: '4.0.18',
  label: 'v4.0.18 ADMIN LIVE ACTIVITY STATUS SYNC'
};
...
(function cp412PlatformLifecycleSyncFix(){
  ...
  async function cp412RefreshPlatformLifecycle(reason = 'timer') {
    if (!cp412IsPlatformCommand()) { cp412StopTimer(); return; }
    if (window.CP412_PLATFORM_LIFECYCLE_BUSY) return;
    window.CP412_PLATFORM_LIFECYCLE_BUSY = true;
    try {
      await loadData();
      if (cp412IsPlatformCommand()) render();
    } catch (err) {
      console.warn('Platform lifecycle sync failed:', err);
    } finally {
      window.CP412_PLATFORM_LIFECYCLE_BUSY = false;
    }
  }
  function cp412EnsureTimer() {
    if (!cp412IsPlatformCommand()) { cp412StopTimer(); return; }
    if (!window.CP412_PLATFORM_LIFECYCLE_TIMER) {
      window.CP412_PLATFORM_LIFECYCLE_TIMER = setInterval(() => cp412RefreshPlatformLifecycle('interval'), 9000);
    }
  }
  ...
  BUILD.version = '4.0.18';
  BUILD.label = 'v4.0.18 ADMIN LIVE ACTIVITY STATUS SYNC';
  window.CP_ACTIVE_BUILD_LABEL = BUILD.label;
  window.CP_DEV_CACHE_BUST = '2026-06-27T0715-v418-admin-live-activity-status-sync';
})();
...
(function cp414MarketplaceActivityGuardStatusFeed(){
  const CP414_VERSION = '4.0.18';
  const CP414_LABEL = 'v4.0.18 ADMIN LIVE ACTIVITY STATUS SYNC';
  const CP414_CACHE = '2026-06-27T0715-v418-admin-live-activity-status-sync';
...
/* v4.0.18 absolute badge hard lock: updates every visible build label after all patches load. */
(function cp415AbsoluteBadgeHardLock(){
  const LABEL = 'v4.0.18 ADMIN LIVE ACTIVITY STATUS SYNC';
  const VERSION = '4.0.18';
  const CACHE = '2026-06-27T0715-v418-admin-live-activity-status-sync';
  function lock() {
    try {
      ...
      if (meta) meta.setAttribute('content', '4.0.18-admin-live-activity-status-sync');
    } catch {}
  }
  lock();
  setTimeout(lock, 0);
...
/* -----------------------------------------------------------------------------
   ...
   - Keeps the Platform Command Center refreshing without requiring manual clicks.
----------------------------------------------------------------------------- */
(function cp418AdminLiveActivityStatusSync(){
  const CP418_VERSION = '4.0.18';
  const CP418_LABEL = 'v4.0.18 ADMIN LIVE ACTIVITY STATUS SYNC';
  const CP418_CACHE = '2026-06-27T0715-v418-admin-live-activity-status-sync';
  const CP418_FEED_LIMIT = 48;
  const CP418_STAGE_ORDER = {
    open_marketplace: 1,
    agency_accepted: 2,
    guard_assigned: 3,
    assigned: 3,
  ...
  const CP418_CORE_STATUSES = ['guard_accepted','en_route','arrived','in_progress','proof_uploaded','completed','report_published'];

  function cp418Status(value = '') {
    ...
    return CP418_STAGE_ORDER[raw] ? raw : raw;
  }
  function cp418StatusFromText(row = {}) {
    const direct = cp418Status(row.event_status || row.current_status || row.next_status || row.status || row.stage || row.event_type || row.action || '');
  ...
  function cp418Order(status = '') { return CP418_STAGE_ORDER[cp418Status(status)] || 0; }
  function cp418Label(status = '') { return (CP418_LABELS[cp418Status(status)] || [statusText?.(status || 'Update') || 'Marketplace Update', 'Marketplace status updated'])[0]; }
  function cp418Sub(status = '') { return (CP418_LABELS[cp418Status(status)] || ['Marketplace Update', 'Marketplace status updated'])[1]; }
  function cp418Time(value) { return value || ''; }
  function cp418SafeId(value = '') { return String(value || '').trim(); }
  function cp418JobId(job = {}) { return cp418SafeId(job.id || job.marketplace_job_id || job.job_id || ''); }
  function cp418JobNumber(job = {}) { return job.job_number || job.request_number || String(cp418JobId(job)).slice(0, 12) || 'Marketplace Job'; }
  function cp418EventJobId(e = {}) { return cp418SafeId(e.job_id || e.marketplace_job_id || e.marketplace_job || ''); }
  function cp418RequestId(row = {}) { return cp418SafeId(row.request_id || row.patrol_request_id || row.requestId || ''); }
  function cp418EventTime(e = {}) { return e.created_at || e.event_at || e.updated_at || e.inserted_at || ''; }
  function cp418CurrentJobs() {
    const rows = (typeof cp409PlatformJobsAll === 'function' ? cp409PlatformJobsAll() : (state.marketplaceJobs || []));
    return Array.isArray(rows) ? rows.slice() : [];
  }
  function cp418JobMaps() {
    const byJob = new Map();
    const byRequest = new Map();
    cp418CurrentJobs().forEach(job => {
      const jid = cp418JobId(job);
      const rid = cp418RequestId(job);
      if (jid) byJob.set(jid, job);
      if (rid) byRequest.set(rid, job);
    });
    return { byJob, byRequest };
  }
  function cp418JobForRow(row = {}, maps = cp418JobMaps()) {
    const jid = cp418EventJobId(row) || cp418SafeId(row.marketplace_job_id || row.job_id || '');
    const rid = cp418RequestId(row);
    return (jid && maps.byJob.get(jid)) || (rid && maps.byRequest.get(rid)) || {};
  }
  function cp418RequestForJob(job = {}) {
    const rid = cp418RequestId(job);
    return (state.patrolRequests || []).find(req => String(req.id || '') === String(rid)) || {};
  }
  function cp418ClientName(job = {}) {
    const req = cp418RequestForJob(job);
  ...
  function cp418ProofRows(maps = cp418JobMaps()) {
    const rows = [];
    (state.proofItems || []).forEach((proof, idx) => {
      const time = proof.uploaded_at || proof.created_at || proof.updated_at || '';
      if (!time) return;
      const job = cp418JobForRow(proof, maps);
      const jid = cp418JobId(job) || proof.marketplace_job_id || proof.job_id || '';
      rows.push({
        id: proof.id || proof.object_path || `proof-${idx}`,
        jobId: jid,
        requestId: cp418RequestId(proof) || cp418RequestId(job),
        status: 'proof_uploaded',
        time,
        title: 'Proof Uploaded',
        message: proof.note || proof.file_name || 'Photo/video proof attached to marketplace job',
        agency: cp418Agency(job, proof),
        guard: cp418Guard(job, proof),
        jobNumber: cp418JobNumber(job),
        property: cp418Property(job),
        client: cp418ClientName(job),
        source: 'proof_items'
      });
    });
    return rows;
  }
  function cp418LocalReports() {
    const reports = [];
    try {
  ...
  function cp418ReportRows(maps = cp418JobMaps()) {
    const rows = [];
    cp418LocalReports().forEach((report, idx) => {
      ...
      if (!time) return;
      const job = cp418JobForRow(report, maps);
      const jid = cp418JobId(job) || report.marketplace_job_id || report.job_id || '';
      rows.push({
        id: report.id || report.report_id || `report-${idx}`,
        jobId: jid,
        requestId: cp418RequestId(report) || cp418RequestId(job),
        status: 'report_published',
        time,
        title: 'Report Published',
        message: report.title || report.report_number || 'Final report released to client',
        agency: cp418Agency(job, report),
        guard: cp418Guard(job, report),
        jobNumber: cp418JobNumber(job),
        property: cp418Property(job),
        client: cp418ClientName(job),
  ...
  function cp418EventRows(maps = cp418JobMaps()) {
    const rows = [];
    (state.jobEvents || []).forEach((event, idx) => {
      const status = cp418StatusFromText(event);
      if (!status || !CP418_STAGE_ORDER[status]) return;
      const time = cp418EventTime(event);
      if (!time) return;
      const job = cp418JobForRow(event, maps);
      rows.push({
        id: event.id || `event-${idx}`,
        jobId: cp418JobId(job) || cp418EventJobId(event),
        requestId: cp418RequestId(event) || cp418RequestId(job),
        status,
        time,
        title: cp418Label(status),
        message: event.details || event.message || event.description || [event.actor_name, cp418Agency(job, event), cp418JobNumber(job)].filter(Boolean).join(' · '),
        agency: cp418Agency(job, event),
        guard: cp418Guard(job, event),
        jobNumber: event.job_number || cp418JobNumber(job),
        property: cp418Property(job),
        client: cp418ClientName(job),
        source: 'job_events'
      });
  ...
  function cp418StageTime(job = {}, status = '') {
    const fields = {
  ...
  function cp418JobRows(maps = cp418JobMaps()) {
    const rows = [];
    cp418CurrentJobs().forEach(job => {
      const id = cp418JobId(job);
      if (!id) return;
      const current = cp418Status(job._platform_lifecycle_status || job.current_status || job.status || 'open_marketplace');
      const currentOrder = cp418Order(current);
      ...
      statuses.forEach(status => {
        let time = cp418StageTime(job, status);
        if (status === 'proof_uploaded' && !time) {
          const proof = cp418ProofRows(maps).filter(r => String(r.jobId || '') === id || (cp418RequestId(job) && String(r.requestId || '') === cp418RequestId(job))).sort((a,b) => new Date(b.time || 0) - new Date(a.time || 0))[0];
          time = proof?.time || '';
        }
        if (status === 'report_published' && !time) {
          const report = cp418ReportRows(maps).filter(r => String(r.jobId || '') === id || (cp418RequestId(job) && String(r.requestId || '') === cp418RequestId(job))).sort((a,b) => new Date(b.time || 0) - new Date(a.time || 0))[0];
          time = report?.time || '';
        }
        if (!time && current === status) time = job.updated_at || job.completed_at || job.created_at || '';
        if (!time || currentOrder < cp418Order(status)) return;
        rows.push({
          id: `${id}-${status}-${time}`,
          jobId: id,
          requestId: cp418RequestId(job),
          status,
          time,
          title: cp418Label(status),
          message: `${cp418Guard(job)} · ${cp418Agency(job)} · ${cp418Property(job)}`,
          agency: cp418Agency(job),
          guard: cp418Guard(job),
          jobNumber: cp418JobNumber(job),
          property: cp418Property(job),
          client: cp418ClientName(job),
          source: 'marketplace_jobs'
  ...
  function cp418Key(row = {}) {
    const jobKey = row.jobId || row.requestId || 'global';
    const status = cp418Status(row.status);
  ...
  function cp418BuildFeedRows() {
    const maps = cp418JobMaps();
    const all = [
      ...cp418EventRows(maps),
      ...cp418JobRows(maps),
      ...cp418ProofRows(maps),
      ...cp418ReportRows(maps)
    ].filter(row => row.time && cp418Status(row.status));
    const best = new Map();
    all.forEach(row => {
      const key = cp418Key(row);
      const old = best.get(key);
    ...
    return Array.from(best.values()).sort((a,b) => new Date(b.time || 0) - new Date(a.time || 0)).slice(0, CP418_FEED_LIMIT);
  }
  function cp418FeedRow(row = {}) {
    const status = cp418Status(row.status);
    const guard = row.guard && row.guard !== 'Unassigned' ? row.guard : '';
    const meta = [guard, row.agency, row.jobNumber, row.property].filter(Boolean).join(' · ');
    const msg = row.message && !String(row.message).includes('undefined') ? row.message : meta;
    ...
    return `<div class="live-gps-feed-row cp414-feed-row cp418-feed-row cp418-feed-${esc(status)} cp414-feed-${esc(status)}"><i></i><span><strong>${esc(fmtTime(row.time))} <em class="cp418-feed-source">${esc(source)}</em></strong><p>${esc(row.title || cp418Label(status))}</p><small>${esc(msg || cp418Sub(status))}</small></span></div>`;
  }
  function cp418CoverageBar() {
    return `<div class="cp418-coverage-bar">${CP418_CORE_STATUSES.map(s => `<span class="cp418-coverage-chip cp418-${esc(s)}">${esc(cp418Label(s))}</span>`).join('')}</div>`;
  }
  if (typeof cp409PlatformActivityFeed === 'function') {
    cp409PlatformActivityFeed = function cp418PlatformActivityFeed() {
      const rows = cp418BuildFeedRows();
      const lastSync = state.platformLifecycleLastSyncAt ? `Synced ${timeAgo(state.platformLifecycleLastSyncAt)}` : 'Live sync';
      return `<section class="panel panel-pad cp409-feed-panel cp412-feed-panel cp414-feed-panel cp418-feed-panel"><div class="panel-head"><div><h2>Marketplace Activity</h2><p>Admin live status sync for every guard step and report publish event.</p></div><button class="ghost-button" data-action="platform-command-refresh-v409">${esc(lastSync)}</button></div>${cp418CoverageBar()}<div class="live-gps-feed-list">${rows.length ? rows.map(cp418FeedRow).join('') : '<div class="empty">No marketplace guard activity yet.</div>'}</div></section>`;
    };
  }
  async function cp418Refresh(reason = 'interval') {
    if (!(typeof isPlatformAdmin === 'function' && isPlatformAdmin()) || state.view !== 'dashboard') return;
    if (window.CP418_ADMIN_SYNC_BUSY) return;
    window.CP418_ADMIN_SYNC_BUSY = true;
    try {
      await loadData();
      state.platformLifecycleLastSyncAt = new Date().toISOString();
      if (typeof render === 'function' && state.view === 'dashboard') render();
    } catch (err) {
      console.warn('v4.0.18 admin live activity sync failed:', err);
    } finally {
      window.CP418_ADMIN_SYNC_BUSY = false;
    }
  }
  function cp418EnsureTimer() {
    if (window.CP418_ADMIN_SYNC_TIMER) return;
    window.CP418_ADMIN_SYNC_TIMER = setInterval(() => {
      if (typeof isPlatformAdmin === 'function' && isPlatformAdmin() && state.view === 'dashboard') cp418Refresh('interval');
    }, 6000);
  }
  function cp418FindJobForRequest(req = {}) {
    const maps = cp418JobMaps();
    const direct = cp418SafeId(req.marketplace_job_id || req.job_id || '');
    if (direct && maps.byJob.get(direct)) return maps.byJob.get(direct);
    return maps.byRequest.get(cp418SafeId(req.id || req.request_id || '')) || null;
  }
  async function cp418SyncReportPublished(req = {}, report = {}) {
    const job = cp418FindJobForRequest(req);
    if (!job?.id) return;
    ...
    const event = {
      id: `cp418-report-published-${job.id}-${Date.now()}`,
      job_id: job.id,
      ...
      metadata: { source: 'v4.0.18_admin_live_activity_status_sync', report_id: report.id || '' },
      created_at: at
    };
    try {
      ...
      console.warn('v4.0.18 report status persistence skipped:', err);
    }
    try {
      ...
      console.warn('v4.0.18 report event persistence skipped:', err);
    }
    try {
  ...
  if (typeof publishClientReport === 'function' && !window.CP418_PUBLISH_PATCHED) {
    window.CP418_PUBLISH_PATCHED = true;
    const cp418BasePublishClientReport = publishClientReport;
    publishClientReport = async function cp418PublishClientReportWithMarketplaceSync(...args) {
      ...
      const result = await cp418BasePublishClientReport.apply(this, args);
      try {
        if (reqBefore?.id) {
          const published = (typeof publishedReportForRequestId === 'function' ? publishedReportForRequestId(reqBefore.id) : null) || reportBefore || {};
          await cp418SyncReportPublished(reqBefore, published);
        }
      } catch (err) {
        console.warn('v4.0.18 report publish activity sync failed:', err);
      }
      return result;
    };
  }
  try {
    BUILD.version = CP418_VERSION;
    BUILD.label = CP418_LABEL;
    window.CP_ACTIVE_BUILD_LABEL = CP418_LABEL;
    window.CP_DEV_CACHE_BUST = CP418_CACHE;
    window.CP418_ADMIN_ACTIVITY_SYNC = { buildRows: cp418BuildFeedRows, refresh: cp418Refresh };
    const meta = document.querySelector('meta[name="app-version"]');
    if (meta) meta.setAttribute('content', '4.0.18-admin-live-activity-status-sync');
    if (typeof ensureBadge === 'function') setTimeout(ensureBadge, 0);
  } catch {}
  cp418EnsureTimer();
  setTimeout(() => cp418Refresh('boot'), 1800);
  document.addEventListener('visibilitychange', () => { if (!document.hidden) setTimeout(() => cp418Refresh('visible'), 250); });
  window.addEventListener('focus', () => setTimeout(() => cp418Refresh('focus'), 250));
  function cp418LockBadge() {
    try {
      BUILD.version = CP418_VERSION;
      BUILD.label = CP418_LABEL;
      window.CP_ACTIVE_BUILD_LABEL = CP418_LABEL;
      window.CP_DEV_CACHE_BUST = CP418_CACHE;
      document.querySelectorAll('.cp-build-badge,.version-mini').forEach(el => { el.textContent = CP418_LABEL; });
      const meta = document.querySelector('meta[name="app-version"]');
      if (meta) meta.setAttribute('content', '4.0.18-admin-live-activity-status-sync');
    } catch {}
  }
  cp418LockBadge();
  setTimeout(cp418LockBadge, 0);
  setTimeout(cp418LockBadge, 300);
  setTimeout(cp418LockBadge, 1000);
  setInterval(cp418LockBadge, 4000);
})();
