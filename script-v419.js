...
const BUILD = {
  version: '4.0.19',
  label: 'v4.0.19 QUIET ADMIN LIVE SYNC NO PAGE RELOAD'
};
...
(function cp412PlatformLifecycleSyncFix(){
  ...
  BUILD.label = 'v4.0.19 QUIET ADMIN LIVE SYNC NO PAGE RELOAD';
  window.CP_ACTIVE_BUILD_LABEL = BUILD.label;
...
(function cp414MarketplaceActivityGuardStatusFeed(){
  ...
  const CP414_LABEL = 'v4.0.19 QUIET ADMIN LIVE SYNC NO PAGE RELOAD';
...
(function cp415AbsoluteBadgeHardLock(){
  const LABEL = 'v4.0.19 QUIET ADMIN LIVE SYNC NO PAGE RELOAD';
  ...
  function lock() {
    try {
      ...
      if (meta) meta.setAttribute('content', '4.0.19-quiet-admin-live-sync-no-page-reload');
    } catch {}
  }
  lock();
  setTimeout(lock, 0);
...
(function cp419AdminLiveActivityStatusSync(){
  const CP419_VERSION = '4.0.19';
  const CP419_LABEL = 'v4.0.19 QUIET ADMIN LIVE SYNC NO PAGE RELOAD';
